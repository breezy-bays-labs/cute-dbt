...
(function () {
  ...
  $(function () {
    ...
    // cute-dbt#350 — the "Data tables" seed section, built before the
    // zero-models early return: a seeds-only PR (no in-scope model) still
    // renders its seed data. JS-built (no static markup) so the default
    ...
    renderRemovedModels();
    // cute-dbt#420 — the top-of-report PR review-comment count button renders
    // once at boot (the total is fixed for the report). No-op when
    // DATA.pr_comments is absent (off-gate / no PR context / no comments).
    renderPrCommentsCountButton();
    renderModelSelector();
    renderTestSelector();
  ...
  // ---- PR review comments (#419 / #420 / #421 / #422, epic #353) -------
  //
  // The whole comment surface is CLIENT-RENDERED from DATA.pr_comments (the
  // pr_dag / seed_cards precedent) — the Rust side already ANCHORED each
  // review thread onto a rendered diff line (the shipped #418 anchoring) and
  // grouped the threads per model, so the JS only PLACES them: the top count
  // button (#420), the per-model count tooltip (#421), and the inline thread
  // at its Model-SQL diff line (#419). Absent the `pr-comments` experiment /
  // no PR context / no comments, DATA.pr_comments is omitted and every
  // function here no-ops — keeping every default golden byte-identical.

  // The per-model comment bucket for model `m`, matched on the model's
  // project-relative file path (`m.path` — the join key BOTH the model
  // payload and the bucket carry; the model payload does NOT carry the node
  // id). Null when the model carries no anchored comments.
  function prCommentBucketFor(m) {
    var pc = DATA && DATA.pr_comments;
    if (!pc || !pc.by_model || !m || !m.path) { return null; }
    for (var i = 0; i < pc.by_model.length; i++) {
      if (pc.by_model[i].model_path === m.path) { return pc.by_model[i]; }
    }
    return null;
  }

  // Find the model NAME owning a comment bucket (so navigation can select it
  // in the <select>, which keys on name). Matches on the bucket's
  // model_path. Null when the bucket's model is not a current dropdown model.
  function modelNameForBucket(bucket) {
    for (var i = 0; i < DATA.models.length; i++) {
      if (DATA.models[i].path === bucket.model_path) { return DATA.models[i].name; }
    }
    return null;
  }

  // #420 — the top-of-report total-count navigation button. Fills the
  // always-present static .pr-comments-count container and unhides it;
  // clicking navigates to the FIRST anchored thread (selects its model +
  // scrolls to the anchored Model-SQL diff line). Rendered ONCE at boot (the
  // total is fixed for the report). Absent comments ⇒ the container stays
  // hidden (byte-identity-safe: it is identical empty markup in every report).
  function renderPrCommentsCountButton() {
    var $btn = $('[data-testid="pr-comments-count"]');
    if (!$btn.length) { return; }
    var pc = DATA && DATA.pr_comments;
    var total = (pc && typeof pc.total === "number") ? pc.total : 0;
    if (!pc || total < 1) { $btn.prop("hidden", true).empty(); return; }
    var label = total === 1 ? "1 PR review comment" : total + " PR review comments";
    $btn.empty()
      .append($("<span>").addClass("pr-comments-count-icon").attr("aria-hidden", "true").text("💬"))
      .append($("<span>").addClass("pr-comments-count-label").text(label))
      .attr("aria-label", label + " — jump to the first comment")
      .attr("data-total", String(total))
      .prop("hidden", false);
    // The first navigable target: the first thread of the first model bucket
    // that maps to a current model (skip buckets whose model is not in the
    // dropdown). Falls back to no-op navigation if none is anchorable.
    $btn.off("click.cuteComments").on("click.cuteComments", function () {
  ...
  // #421 — the per-model comment count, shown via the FOCUSABLE-BUBBLE
  // tooltip (.expect-tooltip + .expect-tooltip-bubble): a focusable <button>
  // whose CSS bubble paints on :hover AND :focus — NEVER a native `title`
  // (hover-delayed, keyboard-unreachable, touch-invisible). aria-label
  // carries the count for AT; the bubble is aria-hidden (never announced
  // twice). Hidden when the selected model has no anchored comments.
  function renderModelCommentCount(m) {
    var $row = $('[data-testid="model-comments"]');
    if (!$row.length) { return; }
    $row.empty();
    var bucket = prCommentBucketFor(m);
    if (!bucket || !bucket.count) { $row.prop("hidden", true); return; }
    var n = bucket.count;
    var word = n === 1 ? "comment" : "comments";
    var aria = n + " review " + word + " on this model";
    var $btn = $("<button>").attr("type", "button")
      .addClass("expect-tooltip model-comment-count-chip")
      .attr("data-testid", "model-comment-count")
      .attr("data-count", String(n))
      .attr("aria-label", aria)
      .text("💬 " + n + " " + word);
    // The bubble: a per-thread summary (author of the first comment + a
    // resolved/outdated marker), revealed on hover/focus.
    var $bubble = $("<span>").addClass("expect-tooltip-bubble model-comment-bubble")
      .attr("data-testid", "model-comment-bubble").attr("aria-hidden", "true");
    bucket.threads.forEach(function (t) {
      var who = (t.comments && t.comments[0] && t.comments[0].author) || "ghost";
      var where = t.outdated ? "outdated"
        : (t.line != null ? "line " + t.line : "—");
      var stateMark = t.resolved ? " · resolved" : "";
      $bubble.append(
        $("<span>").addClass("model-comment-bubble-row")
          .text("@" + who + " · " + where + stateMark)
      );
    });
    $btn.append($bubble);
  ...
  function anchorModelCommentThreads(m) {
    ...
    // renderModelSql rebuild contract — never leak across a model switch).
    $section.find(".pr-comment-thread, .pr-comment-tail").remove();
    var bucket = prCommentBucketFor(m);
    ...
    if (unplaced.length) {
      var $tail = $("<div>").addClass("pr-comment-tail").attr("data-testid", "pr-comment-tail");
      $tail.append(
        $("<p>").addClass("pr-comment-tail-label")
          .text(unplaced.length === 1
            ? "1 comment refers to a line that changed or is not shown:"
            : unplaced.length + " comments refer to lines that changed or are not shown:")
      );
      unplaced.forEach(function (t) { $tail.append(buildCommentThread(t)); });
      // Place the tail after the diff (or the section wrap) so it is visible.
      var $wrap = $section.find(".model-sql-wrap");
      ($wrap.length ? $wrap : $section).append($tail);
    }
  }

  // Build one review thread's DOM — GitHub-style: a header (anchor/line +
  // resolved/outdated badges) over the comment rows (author + body). All
  // strings are set via .text()/textContent (NEVER innerHTML) so a
  // manifest/PR-derived body is drawn as text, never parsed as HTML.
  function buildCommentThread(t) {
    var $thread = $("<div>").addClass("pr-comment-thread")
      .attr("data-testid", "pr-comment-thread")
      .attr("data-thread-key", t.__key || "");
    if (t.resolved) { $thread.addClass("pr-comment-resolved"); }
    if (t.outdated) { $thread.addClass("pr-comment-outdated"); }
    var $head = $("<div>").addClass("pr-comment-thread-head");
    var anchorText = t.outdated
      ? (t.original_line != null ? "outdated · was line " + t.original_line : "outdated")
      : (t.line != null ? "line " + t.line : "comment");
    $head.append($("<span>").addClass("pr-comment-anchor").text(anchorText));
    if (t.resolved) {
      $head.append($("<span>").addClass("pr-comment-badge pr-comment-badge-resolved").text("resolved"));
    }
    if (t.outdated) {
      $head.append($("<span>").addClass("pr-comment-badge pr-comment-badge-outdated").text("outdated"));
    }
    $thread.append($head);
    // Resolved threads collapse: the body lives in a <details>, closed by
    // default, so a resolved conversation is quiet but still inspectable.
    var $body;
    if (t.resolved) {
      var $det = $("<details>").addClass("pr-comment-collapse");
      $det.append($("<summary>").text(
        (t.comments ? t.comments.length : 0) === 1
          ? "1 comment (resolved)"
          : (t.comments ? t.comments.length : 0) + " comments (resolved)"
      ));
      $body = $("<div>").addClass("pr-comment-body");
      $det.append($body);
      $thread.append($det);
    } else {
      $body = $("<div>").addClass("pr-comment-body");
    ...
    (t.comments || []).forEach(function (c) {
      var $c = $("<div>").addClass("pr-comment");
      $c.append($("<span>").addClass("pr-comment-author").text("@" + (c.author || "ghost")));
      $c.append($("<div>").addClass("pr-comment-text").text(c.body || ""));
      $body.append($c);
    });
    return $thread;
  }
  ...
  function renderForSelectedModel() {
    ...
    // cute-dbt#421 — the per-model PR review-comment count tooltip (focusable
    // bubble). No-op when the selected model carries no anchored comments.
    renderModelCommentCount(m);
    renderTestDetails(t);
    renderModelSql(m);
    // cute-dbt#419 — inject the anchored review threads inline at their
    // Model-SQL diff lines (and an outdated/unplaced tail). MUST run AFTER
    // renderModelSql rebuilds the diff for this model. No-op without comments.
    anchorModelCommentThreads(m);
  ...
  function renderOneDiffLine(ln, tokenizeFn, extraClass, hidden, num) {
    ...
    // cute-dbt#419 — carry the old/new line numbers as data attributes so
    // the PR-comment injector (anchorModelCommentThreads) can find the exact
    // rendered diff line a review thread anchors to (Right side → new line,
    // Left side → old line). Decorative positional metadata only (the
    // .diff-gutter precedent); absent the pr-comments surface they are inert.
    var oAttr = (num && num.o != null) ? ' data-oldline="' + num.o + '"' : "";
    var nAttr = (num && num.n != null) ? ' data-newline="' + num.n + '"' : "";
  ...
  // cute-dbt#350 — the "Data tables" section: each in-scope seed's CURRENT
  // table (capped, with an honest "showing N of M rows" note) + its old→new
  // cell-diff (pr-diff arm), reusing the unit-test fixture machinery
  // wholesale (buildFixtureView → the #98/#127 NULL-aware grids + the
  // Current↔Diff toggle + the same vendored DataTables). JS-built (no static
  // markup), so the default HTML carries zero seed bytes — the section is
  // created only when the gated `DATA.seed_cards` payload is present. A seed
  // whose data could not be read (`table` null) renders a LABELED
  // "data unavailable" state, never a silent empty grid (the cute-dbt#126
  // lesson). The section is inserted once, ABOVE the per-model surfaces, so a
  // seeds-only PR (zero in-scope models) still shows its seed data.
  function renderSeedDataTables() {
    var cards = DATA.seed_cards;
    if (!cards || !cards.length) return;
    var $section = $("<section>")
      .addClass("data-tables-section")
      .attr("aria-label", "Data tables")
      .attr("data-testid", "data-tables-section");
    $section.append($("<h2>").text("Data tables"));
    $section.append(
      $("<p>").addClass("data-tables-subtitle").append(
        document.createTextNode(
          "Seed CSVs the engine loads as data. "
        )
      ).append(
        $("<span>").addClass("data-tables-experimental")
          .attr("data-testid", "data-tables-experimental").text("experimental")
      )
    );
    cards.forEach(function (card) {
      $section.append(buildSeedCard(card));
    });
    // Insert above the always-present .error-card landmark (which sits
    // before the per-model surfaces) so the section lands near the top.
    var $anchor = $(".error-card").first();
    if ($anchor.length) {
      $anchor.before($section);
    } else {
      $("body").append($section);
    }
    // The seed grids are visible immediately (no toggle gate), so initialize
    // their DataTables now — initDataTablesIn skips already-hidden tables and
    // is a no-op for the diff-view's lazy-init Current grid.
    initDataTablesIn($section);
  }

  // One seed's card in the "Data tables" section: an identity + lineage
  // header, the config-display chips, and the fixture-view grid (current
  // table capped + the optional cell-diff toggle) OR the labeled
  // data-unavailable state.
  function buildSeedCard(card) {
    var $card = $("<div>")
      .addClass("data-table-card seed-data-card")
      .attr("data-testid", "seed-data-card")
      .attr("data-seed", card.name);
    // Header: the seed name + project-relative path.
    var $head = $("<div>").addClass("seed-data-head");
    $head.append($("<code>").addClass("seed-data-name").text(card.name));
    if (card.original_file_path) {
      $head.append(
        $("<code>").addClass("seed-data-path").text(card.original_file_path)
      );
    }
    $card.append($head);
    // Lineage: the direct downstream models the seed feeds (the immediate
    // blast radius — never a percentage, just the names).
    var feeds = card.feeds_models || [];
    var $lineage = $("<p>")
      .addClass("seed-data-lineage")
      .attr("data-testid", "seed-data-lineage");
    if (feeds.length) {
      $lineage.append(
        document.createTextNode(
          "Feeds " + feeds.length + (feeds.length === 1 ? " model: " : " models: ")
        )
      );
      feeds.forEach(function (name, i) {
        if (i) $lineage.append(document.createTextNode(", "));
        $lineage.append($("<code>").text(name));
      });
    } else {
      $lineage.append(
        document.createTextNode("Feeds no models (no model ref()s this seed).")
      );
    }
    $card.append($lineage);
    // Config-display chips (delimiter / quote_columns / column_types), each
    // shown only when authored — the payload carries a display string or null.
    var $config = buildSeedConfigChips(card);
    if ($config) $card.append($config);
    // The data grid OR the truthful degrade.
    if (!card.table) {
      $card.append(
        $("<p>")
          .addClass("seed-data-unavailable")
          .attr("data-testid", "seed-data-unavailable")
          .text(
            "Data unavailable — the seed CSV could not be read "
            + "(run with --project-root, and ensure the file exists)."
          )
      );
      return $card;
    }
    ...
    if (card.capped) {
      $card.append(
        $("<p>")
          .addClass("seed-data-rowcap")
          .attr("data-testid", "seed-data-rowcap")
          .attr("data-shown", card.shown_rows)
          .attr("data-total", card.total_rows)
          .text(
            "Showing " + card.shown_rows + " of " + card.total_rows
            + " rows (raise [seeds] row_cap to show more)."
          )
      );
    } else {
      $card.append(
        $("<p>")
          .addClass("seed-data-rowcount")
          .attr("data-testid", "seed-data-rowcount")
          .attr("data-total", card.total_rows)
          .text(
            card.total_rows + (card.total_rows === 1 ? " row" : " rows")
          )
      );
    }
    // The grid: reuse the unit-test fixture machinery wholesale — the
    // Current table (given-table class so the shared DataTables init + the
    // reflow path pick it up) plus, on the pr-diff arm, the cell-diff toggle.
    // `card.name` is the column-tooltip owner; no per-column metadata map for
    // seeds (the #240 truthful fallback names the seed).
    $card.append(buildFixtureView(card.table, "given-table", card.diff, null, card.name));
    return $card;
  }

  // The seed's config-display chips, or null when the seed authored none.
  // Each value is a Rust-composed display string (or null); the JS only
  // renders, every interpolation through jQuery .text() (never parsed as
  // HTML — a hostile config value draws as a glyph).
  function buildSeedConfigChips(card) {
    var rows = [
      ["delimiter", card.delimiter],
      ["quote_columns", card.quote_columns],
      ["column_types", card.column_types]
    ].filter(function (r) { return r[1] != null; });
    if (!rows.length) return null;
    var $wrap = $("<p>")
      .addClass("seed-data-config")
      .attr("data-testid", "seed-data-config");
    rows.forEach(function (r) {
      $wrap.append(
        $("<span>").addClass("seed-config-chip")
          .attr("data-key", r[0])
          .append($("<code>").addClass("seed-config-key").text(r[0]))
          .append(document.createTextNode(": "))
          .append($("<span>").addClass("seed-config-val").text(String(r[1])))
      );
    });
    return $wrap;
  }
