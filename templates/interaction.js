...
(function () {
  ...
  function buildFoldToggleBtn(getRoot) {
    ...
    $b.on("click", function (e) {
      e.preventDefault();
      expanded = !expanded;
      setAllFolds(expanded, getRoot());
      $b.text(expanded ? "Collapse all" : "Expand all").attr("aria-pressed", expanded ? "true" : "false");
    });
    return $b;
  }
  ...
  // `copied` class flash, not a text swap). Used in the Model SQL + Model
  // YAML code headers, far right. Inline SVG only (the README §2.4
  // functional-SVG iconography rule): no icon font, nothing external, so the
  // zero-egress gate is untouched. A real focusable <button> with an
  // aria-label (the #146 lesson — never a hover-only affordance).
  ...
  function copyIconBtn(getText) {
    ...
    $b.on("click", function (e) {
      ...
      var ok = function () {
        $b.addClass("copied").attr("title", "Copied");
        setTimeout(function () { $b.removeClass("copied").attr("title", "Copy"); }, 1200);
      };
      var text = getText();
      try {
