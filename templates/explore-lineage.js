  ...
  function filterByModels(ids, label) {
    ...
    cy.batch(function () {
      ...
      present.forEach(function (id) {
        var n = cy.getElementById(id);
        set = set.union(n).union(n.predecessors()).union(n.successors());
      });
      cy.elements().addClass("dim");
      set.removeClass("dim").addClass("trace");
      present.forEach(function (id) { cy.getElementById(id).removeClass("dim").addClass("sel"); });
    });
    highlighted = null;
    hideDetailCard();
    activeFilter = { label: label, matched: present.slice() };
    notifyHighlight(null);
  }
