...
(function () {
  ...
  // cute-dbt#270 — the active reference filter ({ label, matched }) or
  // null. Declared here so clearHighlight (below) can reset it.
  ...
  function clearHighlight() {
    ...
    highlighted = null;
    // cute-dbt#270 — clearing also drops the active reference filter (so a
    // filter chip toggles, and an empty-canvas tap restores all nodes).
    activeFilter = null;
    hideDetailCard();
  ...
  // cute-dbt#270 — render a project-config / var value for display: a
  // string verbatim, everything else compact JSON. Never evaluated (zero-
  // compute); a hostile value still lands through textContent downstream.
  ...
  function renderDetailCard(id) {
    ...
    // cute-dbt#270 — standing config provenance: the dbt_project.yml
    // `models:` config keys that resolve to THIS model (fusion's
    // deepest-match-wins), each with its resolved value and the winning
    // subtree path. A side-map keyed by node id (the seed_tables
    // precedent); absent ⇒ the section is skipped (no project file, or the
    // model inherits no project-level config). textContent only.
    if (codeBearing) {
      var provenance = (data.config_provenance || {})[id];
      if (provenance && provenance.length) {
        card.appendChild(el("p", "detail-section-title", "project config"));
        var provList = el("ul", "detail-provenance", null);
        provenance.forEach(function (p) {
          var li = document.createElement("li");
          var chip = el("span", "detail-config-chip", null);
          chip.appendChild(el("code", null, "+" + p.key + " = " + formatConfigValue(p.value)));
          li.appendChild(chip);
          li.appendChild(document.createTextNode(" "));
          li.appendChild(el("span", "detail-config-via", "via " + p.path));
          provList.appendChild(li);
        });
        card.appendChild(provList);
      }
    }
  ...
  // ---- cute-dbt#270: reference filter (var:NAME / macro:NAME) -------------
  // Dim every node except the matched model set + its lineage (the
  // highlight vocabulary, applied to a SET rather than one node). Composes
  // with the static macro-role dim, in place — never a re-render, never a
  // commit. `clearHighlight` clears it (the same .dim/.trace/.sel classes).
  function filterByModels(ids, label) {
    // Resolve each id to its Cytoscape node ONCE (cute-dbt#433) — the prior
    // form called cy.getElementById up to 3× per id (filter, union, select).
    var present = [];
    (ids || []).forEach(function (id) {
      var n = cy.getElementById(id);
      if (n && n.length) { present.push({ id: id, node: n }); }
    });
    cy.batch(function () {
      cy.elements().removeClass("dim sel trace");
      ...
      present.forEach(function (p) {
        set = set.union(p.node).union(p.node.predecessors()).union(p.node.successors());
      });
      cy.elements().addClass("dim");
      set.removeClass("dim").addClass("trace");
      present.forEach(function (p) { p.node.removeClass("dim").addClass("sel"); });
    });
    highlighted = null;
    hideDetailCard();
    activeFilter = { label: label, matched: present.map(function (p) { return p.id; }) };
    notifyHighlight(null);
  }

  // A `var:NAME` / `macro:NAME` query in the search box resolves against
  // the project pane's filter indices. Returns true when handled as a
  // reference filter (so the fuzzy node search is skipped for that query).
  function tryReferenceFilter(q) {
    var m = /^(var|macro):(.+)$/.exec(q);
    if (!m) { return false; }
    var kind = m[1], name = m[2].trim();
    var index = pane && (kind === "var" ? pane.var_models : pane.macro_models);
    var ids = (index && index[name]) || [];
    filterByModels(ids, kind + ":" + name);
    closeList();
    return true;
  }

  // ---- cute-dbt#270: the project pane (info + vars inventory + filter) ----
  // Reads the embedded `data.project_pane` carrier; absent ⇒ the shell
  // stays hidden (the no-project-root golden shape). createElement /
  // textContent only.
  var pane = data.project_pane || null;
  ...
  function renderProjectPane() {
    ...
    if (pane.vars && pane.vars.length) {
      paneEl.appendChild(renderVarsTable(pane.vars));
      paneEl.appendChild(renderFootprint(pane.vars_footprint));
    } else {
  ...
  function renderVarsTable(vars) {
    ...
    vars.forEach(function (v) {
      ...
      row.appendChild(el("td", null, formatConfigValue(v.value)));
  ...
  function renderFootprint(fp) {
    ...
    return el("p", "project-footprint", note);
  }

  // The reference-filter chips: one per var (filter the graph to its
  // referencing models) and one per referenced macro name. Clicking a chip
  // dims the complement; clicking again (or any clear) restores.
  function renderFilterRow() {
    var varNames = pane.var_models ? Object.keys(pane.var_models) : [];
    var macroNames = pane.macro_models ? Object.keys(pane.macro_models) : [];
    if (!varNames.length && !macroNames.length) { return; }
    paneEl.appendChild(el("p", "project-section-title", "reference filter"));
    var row = el("div", "ref-filter", null);
    varNames.sort().forEach(function (name) {
      row.appendChild(filterChip("var:" + name, pane.var_models[name]));
    });
    macroNames.sort().forEach(function (name) {
      row.appendChild(filterChip("macro:" + name, pane.macro_models[name]));
    });
    paneEl.appendChild(row);
  }

  function filterChip(label, ids) {
    var btn = el("button", null, label);
    btn.type = "button";
    btn.setAttribute("data-ref-filter", label);
    btn.addEventListener("click", function () {
      if (activeFilter && activeFilter.label === label) { clearHighlight(); }
      else { filterByModels(ids, label); }
    });
    return btn;
  }
  ...
  // Search-box `var:`/`macro:` queries route to the reference filter; a
  // plain query keeps the fuzzy node search (the input listener above runs
  // first, so re-check here on Enter via a capture wrapper).
  input.addEventListener("input", function () {
    var q = input.value.trim();
    if (q.indexOf("var:") === 0 || q.indexOf("macro:") === 0) {
      closeList();
    }
  });
  input.addEventListener("keydown", function (ev) {
    ...
    if (q.indexOf("var:") === 0 || q.indexOf("macro:") === 0) {
      ev.preventDefault();
      if (tryReferenceFilter(q)) { input.blur(); host.focus(); }
    }
  }, true);
  ...
  window.CuteExploreLineage = {
    ...
    // cute-dbt#270 — the active reference filter ({ label, matched }) or
    // null, plus a programmatic apply for the headless geometry asserts.
    activeFilter: function () { return activeFilter; },
    filterByReference: function (label) {
      var m = /^(var|macro):(.+)$/.exec(label);
      if (!m || !pane) { return false; }
      var index = m[1] === "var" ? pane.var_models : pane.macro_models;
      var ids = (index && index[m[2]]) || [];
      filterByModels(ids, label);
      return true;
    }
  };
})();
