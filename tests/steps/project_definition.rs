...
/// deepest-match scenario edits; line 14 the on-run-start hook; line 18
/// the dispatch search order (cute-dbt#269) — the patches below address
/// exactly those lines (`concat!` keeps the authored indentation
/// byte-exact — a `\`-continued literal would eat it). Lines 1–11 are
/// pinned: the pre-#269 patches' line numbers depend on them.
...
fn diff_rewrites_hook(world: &mut World) {
    // Line 14 of the canonical file: the `+` side must byte-match it.
    write_patch(
        world,
        &project_patch(
            14,
...
fn diff_reorders_dispatch(world: &mut World) {
    // Line 18 of the canonical file (the search_order flow list).
    write_patch(
        world,
        &project_patch(
            18,
