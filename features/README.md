...
| SC4 | auditability package risk-team-re-runnable | `zero_egress.feature` (headless-network proof + resource-ref lint). **Asset-manifest completeness is a CI invariant** (`cargo-deny` + `cargo test` over `assets/MANIFEST.toml`), NOT a scenario — same bucket as SC5/SC6. |
...
SC5/SC6 are repo/CI invariants, deliberately not Gherkin. PHI-safe
fixture completeness is the same shape — enforced by
`tests/fixtures/MANIFEST.toml` + `cargo test` + CI grep.

## Conventions

- **Synthetic data only** in example tables (PHI-safe invariant). No real
  customer or PHI data, ever.
