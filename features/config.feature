...
Feature: An operator-supplied TOML config reflects in the rendered report
  ...
  Scenario: A custom report.title is reflected in the rendered HTML
    ...
    When I run cute-dbt with --manifest current.json --baseline-manifest baseline.json --out report.html --config config-q3-review.toml
  ...
  Scenario: An absent subtitle omits the subtitle element entirely
    ...
    When I run cute-dbt with --manifest current.json --baseline-manifest baseline.json --out report.html --config config-title-only.toml
    Then the exit code is 0
  ...
  Scenario: A missing --config file is a usage error before parsing
    When I run cute-dbt with --manifest current.json --baseline-manifest baseline.json --out report.html --config does-not-exist.toml
  ...
  Scenario: An invalid-TOML --config file is a usage error before parsing
    ...
    When I run cute-dbt with --manifest current.json --baseline-manifest baseline.json --out report.html --config config-broken.toml
    Then the exit code is non-zero
    And no file "report.html" is written
